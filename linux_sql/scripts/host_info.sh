...
INSERT INTO host_info(id, hostname, cpu_number, cpu_architecture, cpu_model, cpu_mhz, l2_cache, total_mem, record_time) VALUES (
  (DEFAULT, "$hostname", "$cpu_number", "$cpu_architecture", "$cpu_model",
  "$cpu_mhz", "$l2_cache", "$total_mem", "$record_time")
)
END
)
